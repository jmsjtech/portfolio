...
namespace Oasis.Commands {
    public class CommandManager {
        ...
        public bool MoveEntityBy(Entity entity, Point position) {
            ...
            if (GameLoop.World.CurrentMap.GetEntityAt<Monster>(entity.Get<Render>().sce.Position + position) != null) {
                Entity monster = GameLoop.World.CurrentMap.GetEntityAt<Monster>(entity.Get<Render>().sce.Position + position).Value;
                GameLoop.CommandManager.Attack(entity, monster);
                return true;
            }

            if (GameLoop.World.CurrentMap.GetEntityAt<Door>(entity.Get<Render>().sce.Position + position) != null) {
                Entity door = GameLoop.World.CurrentMap.GetEntityAt<Door>(entity.Get<Render>().sce.Position + position).Value;
            ...
            if (GameLoop.World.CurrentMap.GetEntityAt<BlocksMovement>(entity.Get<Render>().sce.Position + position) != null) {
                return false;
            }

            return Helper.Render_Move(entity.Get<Render>(), position.X, position.Y);
        }
        ...
        public bool MoveEntityTo(Entity entity, Point position) {
            ...
            return Helper.Render_MoveTo(entity.Get<Render>(), position);
        }
        ...
        private static void ResolveDeath(Entity defender) {
            ...
            //check for Item and InBackpack

            foreach (Entity item ) {
                if (item.Get<InBackpack>().ownerID  == defender.Get<ID>().id) {
                    item.Attach(new Render { sce = new SadConsole.Entities.Entity(1, 1) });
                    Helper.Render_Init(item.Get<Render>(), defender.Get<Render>().sce.Position, item.Get<Item>().glyph, item.Get<Item>().fg, item.Get<Item>().bg);
                    item.Detach<InBackpack>();
                }
            }
        ...
        public void Pickup(Entity actor, Entity item) {
            ...
            item.Detach<Render>();
            item.Attach(new InBackpack { ownerID = actor.Get<ID>().id });
            GameLoop.UIManager.SyncMapEntities(GameLoop.World.CurrentMap);
        }
        ...
        public void UseDoor(Entity actor, Entity door) {
            ...
            // Handled an unlocked door that is closed
                ...
                Helper.Door_ToggleOpen(door);
                GameLoop.World.CurrentMap._tiles[GameLoop.World.CurrentMap.xy_idx(door.Get<Tile>().pos)].Detach<BlocksVisibility>();
