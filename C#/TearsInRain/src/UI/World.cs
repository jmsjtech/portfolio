...
namespace TearsInRain {
    ...
    public class World {
        ...
        public void PlayerStealth(long UID, int StealthResult, bool ToStealth) {
            ...
            if (ToStealth) {
                GameLoop.World.players[UID].IsCrouched = true;
            } else {
                GameLoop.World.players[UID].IsCrouched = false;
            }
        }
        ...
        public void CalculateFov(Point dir) {
            ...
            var fovMap = new GoRogue.MapViews.LambdaMapView<bool>(100, 100, CurrentMap.IsTransparent);
            ...
            if (GameLoop.World.players.ContainsKey(GameLoop.NetworkingManager.myUID)) { 
                ...
                Console.WriteLine("start: " + start + ", playerRel: " + playerRel);
                ...
                foreach (var spot in lastFov.NewlySeen) {
                    ...
                    if (!SeenTiles.Contains(new Point(spot.X, spot.Y))) {
