...
use super::{Viewshed, Herbivore, Carnivore, Item, Map, Position, WantsToMelee, RunState, 
    Confusion, ParticleBuilder, EntityMoved, LastActed};
...
impl<'a> System<'a> for AnimalAI {
    ...
    fn run(&mut self, data : Self::SystemData) {
        let (mut map, player_pos, player_entity, runstate, entities, mut viewshed, 
            herbivore, carnivore, item, mut wants_to_melee, mut entity_moved, mut lastactions, mut position, mut confused, mut particle_builder) = data;

        if *runstate != RunState::Running { return; }

        // Herbivores run away a lot
        for (entity, mut viewshed, _herbivore, mut pos, mut lastacted) in (&entities, &mut viewshed, &herbivore, &mut position, &mut lastactions).join() {
        ...
        for (entity, mut viewshed, _carnivore, mut pos, mut lastacted) in (&entities, &mut viewshed, &carnivore, &mut position, &mut lastactions).join() {
            if viewshed.visible_tiles.contains(&*player_pos) && lastacted.lastacted + lastacted.speed_in_ms < SystemTime::now().duration_since(UNIX_EPOCH).expect("Clock Error").as_millis() {
                lastacted.lastacted = SystemTime::now().duration_since(UNIX_EPOCH).expect("Clock Error").as_millis();
