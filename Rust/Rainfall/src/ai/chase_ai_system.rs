use specs::prelude::*;
use crate::{LastActed, Chasing, Position, Map, Viewshed, EntityMoved};
...
impl<'a> System<'a> for ChaseAI {
    ...
    fn run(&mut self, data : Self::SystemData) {
        let (mut lastactions, mut chasing, mut positions, mut map, mut viewsheds, mut entity_moved, entities) = data;
        
        let mut targets : HashMap<Entity, (i32, i32)> = HashMap::new();
        let mut end_chase : Vec<Entity> = Vec::new();
        for (entity, lastacted, chasing) in (&entities, &mut lastactions, &chasing).join() {
