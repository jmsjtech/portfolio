...
pub fn draw_ui(ecs: &World, ctx : &mut Rltk) {
    ...
    for (_player, stats, hc) in (&players, &combat_stats, &hunger).join() {
        ...
        match hc.state {
            ...
            HungerState::Normal => {}
            HungerState::Hungry => ctx.print_color(51, 40, RGB::named(rltk::ORANGE), RGB::named(rltk::BLACK), "Hungry"),
            HungerState::Starving => ctx.print_color(51, 40, RGB::named(rltk::RED), RGB::named(rltk::BLACK), "Starving"),
        }
    }
    ...
    ctx.print_color(2, 40, RGB::named(rltk::YELLOW), RGB::named(rltk::BLACK), &depth);
...
fn draw_tooltips(ecs: &World, ctx : &mut Rltk) {
    ...
    if !tooltip.is_empty() {
        ...
        if mouse_pos.0 > 40 {
            ...
            for s in tooltip.iter() {
                ctx.print_color(left_x, y, RGB::named(rltk::BLACK), RGB::named(rltk::WHITE), s);
                let padding = (width - s.len() as i32)-1;
                for i in 0..padding {
                    ctx.print_color(arrow_pos.x - i, y, RGB::named(rltk::BLACK), RGB::named(rltk::WHITE), &" ".to_string());
                }
                y += 1;
            }
            ctx.print_color(arrow_pos.x, arrow_pos.y, RGB::named(rltk::BLACK), RGB::named(rltk::WHITE), &"->".to_string());
        } else {
            let arrow_pos = Point::new(mouse_pos.0 + 1, mouse_pos.1);
            let left_x = mouse_pos.0 +3;
            let mut y = mouse_pos.1;
            for s in tooltip.iter() {
                ctx.print_color(left_x + 1, y, RGB::named(rltk::BLACK), RGB::named(rltk::WHITE), s);
                let padding = (width - s.len() as i32)-1;
                for i in 0..padding {
                    ctx.print_color(arrow_pos.x + 1 + i, y, RGB::named(rltk::BLACK), RGB::named(rltk::WHITE), &" ".to_string());
                }
                y += 1;
            }
            ctx.print_color(arrow_pos.x, arrow_pos.y, RGB::named(rltk::BLACK), RGB::named(rltk::WHITE), &"<-".to_string());
